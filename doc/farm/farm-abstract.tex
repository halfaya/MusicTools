...
It is intended to evolved into a dependently-typed replacement for Euterpea
...
express pitch as a pair of an octave and relative pitch within the ocatve, which is
...
Music in turn can be converted to MIDI and and played on synthesizers. Existing Haskell
MIDI libraries are called via Agda's Haskell FFI. Details can be found in the github
repository.
...
For counterpoint specifically, we plan represent higher species counterpoint
and also to explore automatic generation of species counterpoint,
contrasting the music created to recent work on generating
