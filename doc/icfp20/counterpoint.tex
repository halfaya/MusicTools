...
representing the proof that the two intervals form a valid motion:

\begin{alltt}
\_13 : motionOk (c 5 , per8) (d 5 , per8)
\end{alltt}

\noindent Compared to the error message produced by our
implementation, this lacks the exact information about the motion
between the two intervals.

We would also like to note that the attempt to precisely reporting
musical errors has appeared in the work by \citet{szamozvancev-welltyped}.
They formalize various musical rules, including those for intervals
and motion, using GHC's custom type error feature.
However, they do not aim to fully support species counterpoint,
and thus do not cover as many rules as we do.
